...
async def start_command(update: Update, context: CallbackContext) -> int:
    ...
    try:
        ...
        await update.message.reply_text(
            "Привет, я - искусственный интеллект, который помогает родителям легче взаимодействовать с детьми и радоваться жизни с малышом."
        )
        time.sleep(2)
        await update.message.reply_text("Как вас зовут?")
        user_id = update.message.from_user.id
        context.user_data["personal_info"]["user_id"] = user_id

        return GET_USER_NAME


# @SELFDESTROYING ИМЯ МОЖНО НЕ СПРАШИВАТЬ, ОНО ПРИЛЕТАЕТ В ОТВЕТЕ ОТ ТЕЛЕГРАММА, КОГДА ПОЛЬЗОВАТЕЛЬ ЗАПУСКАЕТ БОТА (В ТОМ ЖЕ ОБЪЕКТЕ, ГДЕ И ID ЮЗЕРА, ТАМ ЖЕ КСТАТИ И ЕГО ЮЗЕРНЕЙМ НАПРИМЕР)
async def get_user_name(update: Update, context: CallbackContext) -> int:
    context.user_data["personal_info"]["user_name"] = update.message.text
    await update.message.reply_text(
        f"Приятно познакомиться, {context.user_data['personal_info']['user_name']}"
    )
    time.sleep(1)
...
async def get_user_email(update: Update, context: CallbackContext) -> int:
    ...
    write_to_json(context.user_data)
